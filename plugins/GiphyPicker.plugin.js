							...
							Bs.util.E("div").set({
								...
								children: [
									Bs.util.E("input").set({
										...
										events: {
											input: this.search.bind(this, 20),
										},
									}),
									Bs.util.E("img").set({
										props: {
			...
			var req = Bs.util.http.get(options, (res) => {
				...
				res.on("end", () => {
					...
					body.data.forEach((gif) => {
						var req1 = Bs.util.https.get(gif.images.downsized_small.mp4, (res1) => {
							//_gif.url, (res1) => {
							var chunks1 = [];
							res1.on("data", (chunk1) => chunks1.push(chunk1));
							res1.on("end", () => {
								let blob = new Blob(chunks1, { type: "image/gif" });
								var elem = Bs.util.E("video").set({
									props: {
										autoplay: true,
										loop: true,
										preload: "auto",
